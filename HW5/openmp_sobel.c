...
int main( int argc, char **argv )
{
    ...
    #pragma omp parallel private(nthreads,tid, sum1, sum2,offset,magnitude,elms,block)
    {
        tid = omp_get_thread_num();
        nthreads = omp_get_num_threads();
        block = (ysize-1)*(xsize-1)/nthreads;
        printf("Size %d\n",(ysize-1)*(xsize-1));
        printf("%d %d\n",block*tid,block*(nthreads-tid));
        //for (elms = 0 + (block*tid); elms < (ysize-1)*(xsize-1) - (block*tid) ; elms++) {
        for (elms = 0 + (block*tid); elms < (block*(nthreads-tid)) ; elms++) {
            j = elms % (xsize-1) + 1;
            i = elms/(xsize-1) + 1;
            //printf("%d %d %d %d\n",i,j,xsize,ysize);
            offset = i*xsize + j;
