...
layout_t * parser_parseNetlist(char * netlist_n) {
    ...
    if (netlist_f) {
        ...
        layout = (layout_t*)calloc(1,sizeof(layout_t));
...
layout_t* parser_createLayout(layout_t * layout, char ** init) {
    ...
    layout->all_ports = (port_t*)calloc(layout->size_ports, sizeof(port_t)); 
    layout->all_gates = (gate_t*)calloc(layout->size_gates, sizeof(gate_t)); 
    layout->all_wires = (wire_t*)calloc(layout->size_wires, sizeof(wire_t)); 
    int i; 
    for (i = 0; i < layout->size_wires; i++) {
        char ** buffer= &(layout->all_wires[i].name);
        *buffer = (char*)malloc(i/10+2); 
        sprintf(*buffer,"%d",i);
    } 
    layout->grid = (gate_t**)calloc(layout->x_size*layout->y_size,sizeof(gate_t *));
    layout->all_hier = (hier_t*)calloc(sizeof(hier_t),1);
    char * top_n = "top";
    layout->all_hier->label = (char*)calloc(sizeof(char),4);  
...
void parser_linkGate(layout_t *layout, wire_n wiren, gate_n gaten) {
    ...
    gate_n * new_gate_bfr = (gate_n*)malloc(sizeof(gate_n) * sz);  
...
layout_t* parser_addGate(layout_t *layout, char ** init) {
    int i = 1; 
    int fanin_size = 0; 
    wire_n * fanin_bfr = (wire_n*)malloc(sizeof(wire_n)*20);
    ...
    while (init[i]) { 
        if (strstr(init[i],"name=")) {
            ...
            *t_ptr = (char*)calloc(sizeof(char),size); 
    ...
    self_ptr->fanin = (wire_n*)malloc(sizeof(wire_n)*fanin_size); 
...
void parser_linkPort(layout_t *layout, wire_n wiren, port_n portn) {
    ...
    port_n * new_port_bfr = (port_n*)malloc(sizeof(port_n) * sz);  
    //memcpy of 0 bytes is actually undefined behavior
    //But I can't see why std libs would break this
...
short parser_assignHier(layout_t*layout,wire_t* wire,char* hier) {
    ...
    last->next = (hier_t*)calloc(sizeof(hier_t),1); 
    int hier_len = strlen(hier);
    char * str = (char*)calloc(sizeof(char*),hier_len); 
